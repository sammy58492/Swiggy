...
function InsertQueryProductOption($table,$fields,$CFG = 'empty')
	{
	//get the id last secuence, with this we will get the next id seq and take it (id_sec will incriment with this query)
	$link = ConnectDB($CFG);
	$id = -1;
	...
	pg_prepare($link,'sql',"SELECT setval('".$table."_id_seq', (SELECT MAX(id) FROM ".$table."))");
	pg_execute($link,'sql',array());

	pg_prepare($link,'sql1',"SELECT nextval('".$table."_id_seq') as key");
