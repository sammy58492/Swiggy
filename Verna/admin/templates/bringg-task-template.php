<?
       ...
       while($conf = pg_fetch_array($configResult))
               ...
               /*echo $cur_record['name'];
			   echo "1";
					 echo	$cur_record['bringgcompanyid'];
					  echo "2";
					 echo	 $restaurant_address;
					  echo "3";
					 echo	 $cur_record['tel'];
					  echo "4";
					 echo	 $cur_record_location->latitud;
					  echo "5";
				      echo   $cur_record_location->longitud;
					   echo "6";
					 echo	  $setting['BRINGG_ACCESS_TOKEN'];
					  echo "7";
					 echo	date('Y-m-d H:i:s');*/
						
				$data_string_bus = array(
						 'name' => $cur_record['name'],
						 'company_id' => $cur_record['bringgcompanyid'],
		...
		/*********************************************customers********************************************************/					
			...
			$url = 'http://api.bringg.com/partner_api/tasks';
			   $driver_delivery_time = "UTC + ".$setting['SET_DRIVER_DELIVERY_TIME']." min";
				...
				if($return_data->task->id) {	
			      //  echo $return_data->task->id;	
					...
					$cus_del_time = $setting['SET_DRIVER_DELIVERY_TIME']+30;
...
/*****************************************	for Notes *****************************************************************/						
	...
	/*****************************************	for Notes *****************************************************************/					
				...
				pg_prepare($link,'sqlos1','UPDATE w_orders SET bringg_order_id=$1, bringg_customer_id=$2, bringg_company_id=$4 WHERE id=$3');
				pg_execute($link,'sqlos1',array($return_data->task->id,$return_data->task->customer->id,$id,$cur_record['bringgcompanyid']));
