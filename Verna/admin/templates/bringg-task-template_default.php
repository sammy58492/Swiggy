     ...
     //           }
       ...
       while($conf = pg_fetch_array($configResult))
			   ...
			   if($order->buyer->country !="") {
				...
				$restaurant_address = "";
				  if(isset($cur_record['street']) && $cur_record['street'] !="" )  {
					  
				 $restaurant_address .= $cur_record['street'];	  
				  }
				
				 if(isset($cur_record['colony']) && $cur_record['colony'] !="" )  {
				 $restaurant_address .= ",".$cur_record['colony'];
				  }
				
				 if(isset($getcity['city']) && $getcity['city'] !="" )  {
				 $restaurant_address .= ",".$getcity['city'];
				 }
				  if(isset($cur_record['cp']) && $cur_record['cp'] !="" )  {
				 $restaurant_address .= ",".$cur_record['cp'];
				 }
				  if(isset($getcountry['name']) && $getcountry['name'] !="" )  {
				 $restaurant_address .= ",".$getcountry['name'];
				  }
		...
		/*********************************************customers********************************************************/					
			...
			if($setting['SET_DRIVER_DELIVERY_TIME'] == '')
				 $setting['SET_DRIVER_DELIVERY_TIME']=0;
				 
			//	 echo "Watting time". $setting['SET_DRIVER_DELIVERY_TIME'];
				date_default_timezone_set('UTC');
			   $driver_delivery_time = "+ ".$setting['SET_DRIVER_DELIVERY_TIME']." min";
                $newScedule = strtotime($driver_delivery_time);

				$deliverDate = date("Y-m-d",$newScedule);
				$deliverhour = date("H:i:s.u",$newScedule);
		...
		//	print_r($data_string);
				...
				if($return_data->task->id) {	
			      //  echo $return_data->task->id;	
					...
					$cus_del_time = $setting['SET_DRIVER_DELIVERY_TIME']+20;
					
					 $driver_delivery_time_cus = "UTC + ".$cus_del_time." min";
						$newSceduleOfUser = strtotime($driver_delivery_time_cus);
						/**/
						$deliverDateOfUser = date("Y-m-d",$newSceduleOfUser);
						$deliverhourOfUser = date("H:i:s.u",$newSceduleOfUser);
