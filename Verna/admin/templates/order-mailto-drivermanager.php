...
$msgDriverManger ='<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
		...
		<body>
			<table width="130" border="0"  cellpadding="0" cellspacing="0">
			  ...
			  <tr><td width="50%">';
					...
					{
                      $msgDriverManger .=   '<img src="http://'.$_SERVER['HTTP_HOST'].'/panel/images/logo/1/normal.jpg" border="0" width="400" height="50"  />';
					}
					else
					{
						 $msgDriverManger .=   '<img src="http://'.$_SERVER['HTTP_HOST'].'/panel/images/logo/1/'.$_SESSION['scriptid'].'/normal.jpg" border="0" width="400" height="50"  />';
					}
                   $msgDriverManger .=  '</td>';
		  
		  if($fblink){
		  
				 $msgDriverManger .='<td><a href="https://www.facebook.com/'.$fblink.'" target="_blank"><img src="http://'.$_SERVER["HTTP_HOST"].'/images/email/Facebook.png"  border="0" /></a></td>';
				 
		  }
			 if($twlink){	
				
				$msgDriverManger .='<td><a href="http://www.twitter.com/'.$twlink.'" target="_blank"><img src="http://'.$_SERVER["HTTP_HOST"].'/images/email/Twitter.png"  border="0" /></a></td>';
			 }
				
				if($rsslink){	
				
				$msgDriverManger .='<td><a href="'.$rsslink.'" target="_blank"><img src="http://'.$_SERVER["HTTP_HOST"].'/images/email/Feed.png"  border="0" /></a></td>';
				
				}
			if($gpluslink){	
				
				$msgDriverManger .='<td><a href="'.$gpluslink.'" target="_blank"><img src="http://'.$_SERVER["HTTP_HOST"].'/images/email/Google+.png"  border="0" /></a></td>';
				
				}
				if($linkendinlink){	
				
				$msgDriverManger .='<td><a href="'.$linkendinlink.'" target="_blank"><img src="http://'.$_SERVER["HTTP_HOST"].'/images/email/LinkedIn.png"  border="0" /></a></td>';
				
				}		
				if($instagramlink){	
				
				$msgDriverManger .='<td><a href="'.$instagramlink.'" target="_blank"><img src="http://'.$_SERVER["HTTP_HOST"].'/images/email/instagram.png"  border="0" /></a></td>';
		 ...
		 $msgDriverManger .='</tr>
			   ...
			   $msgDriverManger .='<table width="570" border="0" cellspacing="0" cellpadding="0" align="center" bgcolor="#DDDDDD">
		  <tr><td>
		<table width="560" style="border:0px solid #e2dbdb;" align="center" cellspacing="0" cellpadding="4" bgcolor="#F7F7F7">
		  <tr><th style="border:0px solid #e2dbdb;" colspan="5"><span style="font-family:Arial,Georgia,sans-serif;font-size:18px;color:#df2226;line-height:22px;">
		  '.$lang_resource['EMAIL_TEMPLATE_DETAILS_OF_CUSTOMER_ORDER'].':'.$order->id.'</span></th></tr>
		   <tr><th style="border:0px solid #e2dbdb;" colspan="5"><span style="font-family:Arial,Georgia,sans-serif;font-size:18px;color:#df2226;line-height:22px;">
		  '.$lang_resource['EMAIL_TEMPLATE_THIS_ORDER_ASSIGNED_TO_DRIVER'].':'.$driver_name.'</span></th></tr>
		  <tr>
			 ...
			 $msgDriverManger .='<tr>
			  ...
			  if(in_array('Full Address', $checkoutfields)){
			   ...
			   if($data->buyer->reference && in_array('Where did you find about us', $checkoutfields)) {
				$msgDriverManger .= ' <tr><td style="border-left:1px solid #e2dbdb; border-bottom:1px solid #e2dbdb;">'. $lang_resource['DELIVERY_REFERENCE_V2'] . '</td>
