...
//	exit;
	  ...
	  $buname=$data->business[0]->name;
	  $cardname=$data->buyer->name;
	  $cardno=$data->buyer->cardsavecardno;
	  $exdmm=$data->buyer->cardsaveexpmm;
	  $exdyy=$data->buyer->cardsaveexpyy;
	  $email=$data->buyer->email;
	  $tel=$data->buyer->tel;
	  $scode=$data->buyer->cardsavecvv;
	  $address=$data->buyer->address;
	  $colony=$data->buyer->colony;
	  $cityname=$data->buyer->cityname;
	  $postcode=$data->buyer->zip;
	  $state=$data->buyer->state;
	   $total=round($data->total*100);
	...
	/**************************/
	 ...
	 $panelsetting = $row2['value'];
	  ...
	  if($panelsetting == 1){	
	  $MerchantID=$fetchData1['cardsaveid'];
	  $Password=$fetchData1['cardsavepass'];
	  }else{
		  
		if($fetchData1['cardsaveid'] =="" || $fetchData1['cardsavepass'] ==""){
			$all = "-1";
			...
			$result2 = pg_execute($link,'sql2pay'.$bu,array($bu,8));
			if(pg_num_rows($result2)>0){
			$row = pg_fetch_array($result2);			
			$credential = json_decode($row['credential']);
			$flag=true;
			foreach($credential as $cre){
				if($cre->paymentfield =='cardsaveid'){
					$MerchantID = $cre->value;
					$flag=false;
				}
				if($cre->paymentfield =='cardsavepass'){
					$Password = $cre->value;
					$flag=false;
				}					
			}
			if($flag==true){
				//all
				$result3 = pg_execute($link,'sql2payall',array($all,8));
				$row2 = pg_fetch_array($result3);			
				$credentialall = json_decode($row2['credential']);
				foreach($credentialall as $creall){
				if($creall->paymentfield =='cardsaveid'){
					$MerchantID = $creall->value;
				}
				if($creall->paymentfield =='cardsavepass'){
					$Password = $creall->value;
				}	
...
$Amount = $total; //Amount must be passed as an integer in pence
$CurrencyCode = "826"; //826 = GBP

$OrderID =$id;// $_POST['OrderID'];
$OrderDescription = "Order " . $id . "/". $buname; //Order Description for this new transaction

$CardName = stripGWInvalidChars($cardname);
$CardNumber = $cardno;
$ExpMonth = $exdmm;
$ExpYear = $exdyy;
$CV2 = $scode;
...
$Address1 = stripGWInvalidChars($address);
$Address2 = stripGWInvalidChars($colony);
$Address3 ="";// stripGWInvalidChars($_POST['Address3']);
$Address4 = "";//stripGWInvalidChars($_POST['Address4']);
$City = stripGWInvalidChars($cityname);
$State = stripGWInvalidChars($state);
$Postcode = stripGWInvalidChars($postcode);



$Country = stripGWInvalidChars('826');
$EmailAddress = stripGWInvalidChars($email);
$PhoneNumber = stripGWInvalidChars($tel);
	...
	if($err == 0) {
		...
		if(is_numeric($StatusCode)) {
			...
			if( $StatusCode != 30 ) {
				...
				$id=$OrderID;
				$txtcode=$szAuthCode.",".$szAddressNumericCheckResult;
				$string="host=" . $CFG->dbhost . " dbname=" . $CFG->dbname . " user=" . $CFG->dbuser . " password=" . $CFG->dbpass;
				$link = pg_connect($string);
				
				pg_prepare($link,'sqlu','UPDATE w_orders SET merchantval=$1 WHERE id=$2');
				pg_execute($link,'sqlu',array($txtcode,$id));
				...
				if($_REQUEST['siteFrom']=="mobile"){
						echo "<script>window.location='".$WebAddress."/panel/payment-gateway/worldpay/marchent_return.php?siteFrom=$siteFrom&id=$OrderID'</script>";
						break;	
				}else{
					echo "<script>window.location='".$WebAddress."/panel/payment-gateway/worldpay/marchent_return.php?id=$OrderID'</script>";
					break;	
					}
