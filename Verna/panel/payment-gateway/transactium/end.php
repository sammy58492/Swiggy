...
<?php
	...
	else
	{
		...
		?>
		<?php /*?>Transaction ID: <b><?php echo ($_SESSION["hpsid"]); ?></b><br>
		Status: <b><?php echo($payment->GetHostedPaymentResult->Status); ?></b><?php */?>
		<?php
		

if($payment->GetHostedPaymentResult->Status=='Failure'){
$paymentid = $payment->GetHostedPaymentResult->Status;
}else{
$paymentid = $_SESSION["hpsid"].""."('".$payment->GetHostedPaymentResult->Status."')";
}
...
if($payment->GetHostedPaymentResult->Status!='Failure'){
pg_prepare($link,'sql','UPDATE w_orders SET transactium_tid=$1,transactium_status=$2 WHERE id=$3');
pg_execute($link,'sql',array($_SESSION["hpsid"],$payment->GetHostedPaymentResult->Status,$id));
}
...
#####Use Mail#########
echo $Showmsg; 
SendMail($msg,$msg_pdf,$lang_resource['PAYMENT_GATEWAY_ALL_TRANSACTIUM_SAVE_SUCCESS'],$emails,$id,$link);
//SendMail($msg,$lang_resource['PAYMENT_GATEWAY_ALL_TRANSACTIUM_SAVE_SUCCESS'],$emails);
