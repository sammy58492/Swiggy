...
if(!isset($_SESSION['order_id'])) {
	...
	if(isset($JSON["paymentId"]) && $JSON["paymentId"] > 0) {
		$_SESSION['order_id'] = $JSON->paymentId;
	}
}
...
if(isset($_SESSION['order_id']))
{
	 ...
	 //update field paypaltx for success
		pg_prepare($link,'sqlmakeskus','UPDATE w_orders SET paypaltx = $1;');	
		pg_execute($link,'sqlmakeskus',array($_SESSION['order_id']));
		//update ends here
...
pg_prepare($link,'sqlipn1','SELECT * FROM w_orders WHERE id = $1;');
		...
		if($reQ['paypaltx'])
		{
			...
			pg_prepare($link,'sql_delete_order','Delete * from w_orders WHERE order_id=$1');
			$result = pg_execute($link,'sql_delete_order', array($id));
		}
...
#####Use Mail#########
echo $Showmsg; 
SendMail($msg,$msg_pdf,$lang_resource['PAYMENT_GATEWAY_ALL_MAKESKESKUS_SAVE_SCCESS'],$emails,$id,$link);
