 ...
 <?php
 //print_r($_REQUEST);
$_REQUEST["ResponseCode"] = '00';
//$_REQUEST["AuthorizationCode"] ='XDFFD677656A';
   if($response=$_REQUEST["ResponseCode"] !="00")
  {
...
pg_prepare($link,'sqlpayu','UPDATE w_orders SET bsa_result=$1 WHERE id=$2');
pg_execute($link,'sqlpayu',array($_REQUEST["__atuvs"],$_REQUEST['id']));
...
//print_r($search_row);
	if($search_row == 1){
		...
		$result = pg_execute($link,'sql33',array($order->buyer->cityid));
...
SendMail($msg,$msg_pdf,$lang_resource['PAYMENT_GATEWAY_ALL_BSA_PAYMENT_SUCCESS'],$emails,$id,$link);
