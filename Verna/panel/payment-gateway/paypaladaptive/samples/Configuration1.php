...
class Configuration
{
	...
	public static function getAcctAndConfig()
	{
		...
		$result = pg_execute($link,'sql',array('pusername'));
		$row = pg_fetch_array($result);
		$UserName= $row['value'];
		
		
		$link3 = ConnectDB();
		pg_prepare($link3,'sql3','SELECT value FROM w_configs WHERE name=$1');
		$result3 = pg_execute($link3,'sql3',array('ppassword'));
		...
		$link3 = ConnectDB();
		pg_prepare($link3,'sql3','SELECT value FROM w_configs WHERE name=$1');
		$result3 = pg_execute($link3,'sql3',array('psign'));
		$row3 = pg_fetch_array($result3);
		...
		$link3 = ConnectDB();
		pg_prepare($link3,'sql3','SELECT value FROM w_configs WHERE name=$1');
		$result3 = pg_execute($link3,'sql3',array('pappid'));
		$row3 = pg_fetch_array($result3);
		$AppId= $row3['value'];
