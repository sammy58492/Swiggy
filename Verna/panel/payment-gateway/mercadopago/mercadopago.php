...
class MPRestClient {
    ...
    private static function get_connect($uri, $method, $content_type) {
        if (!extension_loaded ("curl")) {
            throw new Exception($lang_resource['PAYMENT_GATEWAY_ALL_CURL']);
        }

        $connect = curl_init(self::API_BASE_URL . $uri);

        curl_setopt($connect, CURLOPT_USERAGENT, $lang_resource['PAYMENT_GATEWAY_ALL_PHP_SDK_V'] . MP::version);
