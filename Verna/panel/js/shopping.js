...
var IS_GLOBAL_ENABLED = 1;
var IS_BTRANS_ENABLED = 1;
var IS_BSA_ENABLED = 1;
...
var Shopping = {
	...
	lastid = "";
		...
		{   
			...
			var m = Main.GetPropertyValueOnPropertyValueFoundForIds(Main.cuisine, "ids", Main.RedirectToCategory,"ids");			
			category = m; 
		    var business_id = 0;
            ...
            if (a != "")
            {
                ...
                if (q > 0)
                {
                    ...
                    for (p in Shopping.Business)
                    {
						...
						var ptnew= Main.GetPropertyValueOnPropertyValueFoundForIds(Shopping.CategoriesCustom, "ids", r[act], "name");
	...
	if(!Main.RedirectToBusiness){}else{offset='-1'}///////////// for custom slug when businesslist previous and next function////////////
		...
		//alert(offset)
		//alert(JSON.stringify(Main.WhereAmIData))
        $.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchSettingFront"},{"operation":"FetchAllbusinessSettingDataCommon"},{"operation":"FetchItemPointPermission"},{"operation":"FetchDecimalPoint"},{"operation":"FetchNeighbourhoodSettings"},{"operation":"FetchDecimalPoint"},{"operation":"FetchUserInfo"},{"operation":"FetchMapLatlong"},{"operation":"FetchCheckoutInfo"},{"operation":"FetchAllCategories2","businessid":' + businessid +'},{"operation":"FetchAllCategoriesCustom2","businessid":' + businessid +'},{"operation":"FetchCurrentDate"},{"operation":"FetchAllBusinessDataCustomSlug","location":' + JSON.stringify(Main.WhereAmIData.location) + ',"deliverytype":' +  JSON.stringify(Main.WhereAmIData.collecttype) + ',"category":' + JSON.stringify(category) + ',"city":' + business_id+ ',"businessid":' + businessid + ',"whereall":' + JSON.stringify(Main.WhereAmIData) +"}]", function (a)
        {
            ...
            if (a != "")
            {
				...
				if(a.itempointpermisson)
				{
					Main.ItemPointPermission=a.itempointpermisson
					
				}
				
				Main.businessSetting =a.businessSetting;
				if (a.user){
				Main.User = a.user;				
				if(Main.User){
	...
	if(!Main.RedirectToBusiness){}else{offset='-1'}///////////// for custom slug when businesslist previous and next function////////////
        ...
        $.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchCurrentDate"},{"operation":"FetchAllBusinessData1","location":' + JSON.stringify(Main.WhereAmIData.location) + ',"deliverytype":' +  JSON.stringify(Main.WhereAmIData.collecttype) + ',"category":' + JSON.stringify(category) + ',"city":' + business_id + ', "limit":' + limit +',"offset":' + offset + ',"whereall":' + JSON.stringify(Main.WhereAmIData) +"}]", function (a)
        {
            ...
            {	
				a = JSON.parse(a);
              	var pg = a.business.paginationdetails;
                var u = a.business.newbusiness;
				//Main.servicefeesettings = u[0].servicefeesettings;
				...
				if(pg[1]==0){
					//alert('ok');
					 ...
					 if(pg[0]>0){
					//if(p0 < pg[0]){
					//alert('no reslut found in this search. Search for next')
					//Shopping.Nextresult(); 		
					//}else{
				...
				Main.currentDate = a.currentDate;
					...
					$.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchAllCategories1","bids":' + bids +',"offset":' + offset+',"deliverytype":' + JSON.stringify(Main.WhereAmIData.collecttype)+'},{"operation":"FetchAllCategoriesCustom1","bids":' + bids +',"offset":' + offset+',"deliverytype":' + JSON.stringify(Main.WhereAmIData.collecttype)+'}]', function (h)
        {
				//alert(h)
				h = JSON.parse(h);
			    Shopping.CategoriesCustom =  h.categoriesCustom;
                    ...
                    for (var p in h.categories)
                    {
                        ...
                        for (var b in u)
                        {
                            ...
                            for (var s in t)
                            {
								...
								catnew = h.categories[p].ids.split(",")                                                                                                        
                                if (ProductOption.in_array(t[s],catnew))
                                {
                                    c = true
                                }
                        ...
                        if (c)
                        {
                            h.categories[p].enabled = true;
                            r.push(h.categories[p])
                        }
                    }
...
///////////// for businesslist previous and next function////////////		
    ...
    UpdateCartUserInfo: function (d)
    {
        if (!Shopping.Cart)
        {
			...
			if (Main.WhereAmIData)
			{
				...
				if(IS_SCRIPTID == 6){
				Shopping.Cart.buyer.cp = Main.WhereAmIData.address;
				}else{
	...
	var shss = "";
  	shss += '<a href="javascript:Shopping.OpenBusiness('+currentshop+')"><span class="step-logo"><img src="panel/<?=$moduleName?>/images/place_order.png"></span><span class="step-text"><?=$lang_resource['MOBILE_BUSINESS_LIST_PLACE_ORDER'];?></span></a>';
	...
	$.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchAllbusinessSettingData"}]', function (a){
		...
		Main.businesspageheadersetting = b.businessSetting.businesspageheadersetting;
		if(viewDevice == "Desktop") {
			...
			if(Shopping.Business[0].businesspagecustomtext!=""){
				if(document.getElementById("footcustomtxt"))
		...
		$.post("panel/lib/front-bulk.php", 'data=[' + operation + '{"operation":"GetWebsiteReviewData"},{"operation":"FetchAllFranchisesData"},{"operation":"FetchServiceFeeData"},{"operation":"FetchBusinessMenu","businessid":'+d+',"deliverytype":' + JSON.stringify(delTyle) + ',"whereall":'+JSON.stringify(Main.WhereAmIData)+'},{"operation":"timescdule","businessid":' + d + '},{"operation":"pickupDeliverytime","businessid":' + d + '},{"operation":"FetchReserve","businessid":' + d + ',"whereall":'+JSON.stringify(Main.WhereAmIData)+'},{"operation":"checkDelivery","businessid":' + d + '},{"operation":"FetchReserveBooked","businessid":' + d + ',"whereall":'+JSON.stringify(Main.WhereAmIData)+'},{"operation":"DiscountOffer","businessid":' + d + "}]", function (a)
       {
		   ...
		   /* alert(JSON.stringify(a)) */
            ...
            if (a != "")
            {
				...
				Shopping.ReviewWebsite = a.websitereview;
                ...
                Main.FranchisesBackup = a.franchises;
				...
				{ 
					...
					Main.customslug = customslug
					if(!Main.customslugcounter){
						Main.customslugcounter = 0; 
					}
   ...
   FavList: function()
	...
	currentshop = d;
	var shss = "";
   shss += '<a href="javascript:Shopping.OpenBusiness('+currentshop+')"><span class="step-logo"><img src="panel/<?=$moduleName?>/images/place_order.png"></span><span class="step-text"><?=$lang_resource['SHOPPING_PLACE_ORDER']?></span></a>';  
	...
	$.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchAllbusinessSettingData"}]', function (a){
		...
		$.post("panel/lib/front-bulk.php", 'data=[' + operation + '{"operation":"FetchAllFranchisesData"},{"operation":"FetchServiceFeeData"},{"operation":"FetchReserve","businessid":' + d + ',"whereall":'+JSON.stringify(Main.WhereAmIData)+'},{"operation":"FetchReserveBooked","businessid":' + d + '},{"operation":"FetchCurrentDate"},{"operation":"FetchBusinessPreOrderMenu","businessid":' + d + ',"date":' + dd + ',"hour":' + hh + ',"minute":' + mm + ',"deliverytype":' + JSON.stringify(delTyle) + ',"whereall":'+JSON.stringify(Main.WhereAmIData)+'},{"operation":"checkpreorder","menuid":' +  Main.itemid + '},{"operation":"DiscountOffer","businessid":' + d + "}]", function (a)
       {
		//alert(JSON.stringify(a));
		   ...
		   Shopping.Cart.preorderDate =datedd;
	...
	reviewprint: function (f)
	{
		...
		var n = '';
				...
				else
				{
				 ...
				 n +='</div>';
               n +='<div class="dsp-dv">';
                   		n +='<h4>'+f[x].user+' <span><?=$lang_resource['REVIEWS_HOME']?></span> '+Shopping.Business[0].name+'</h4>';
            ...
            n +='</div>';
               ...
               // n +='</table>';
			//  alert(JSON.stringify(Shopping.allphotouser))
    ...
    AddToCart: function (x, y,options,comments,optionsid,total_price,Qtyi,ingredients){
		...
		if(Main.businessSetting.available_product == 1 && IS_SCRIPTID == 0 && ISIFRAME==0){	
			var oldval = document.getElementById(Main.stock_id).value;
			if(oldval < Main.stock_qty1 || Main.newqtyval <= Main.stock_qty1){
				if(Main.newqtyval){
					document.getElementById(Main.stock_id).value = Main.newqtyval;
				}else{
					document.getElementById(Main.stock_id).value = parseInt(oldval) + 1;
				}
				Main.countQty = document.getElementById(Main.stock_id).value;
			}else{
				//alert("<?= $lang_resource['PRODUCT_OPTIONS_AVAILABLE_STOCK'] ?>"+Main.stock_qty1);
				alert("<?= $lang_resource['PRODUCT_OPTIONS_OUT_OF_STOCK'] ?>");
		...
		if(viewDevice == "Mobile") {
			...
			if(IS_SCRIPTID == 0){		
				Shopping.ShowDishAddedGlobe($("#dish_"+y+"_imglink").offset());			
			}
		}
		...
		if(x.id == 'excheck'){
			Main.excheck = x.id;
			var v = Main.GetIndexOnPropertyValueFound(Shopping.Business, "id", Shopping.ActiveBusiness);

			if(!Main.expressService)
				w.expressService =	Shopping.Business[v].expresscheckprice;

			Main.expressService =	Shopping.Business[v].expresscheckprice;
		}else{
			w.id = J.id;
			w.name = J.name;
			w.delivery_price = J.delivery_price;
			w.psettingval = J.psettingval;
			...
			if(total_price) { 	
				if(J.psettingval==1){
					J.delivery_price=Main.NullToEmpty(J.delivery_price);
					if(Main.deliveryType == 'delivery' && J.delivery_price !='' ){		
						tsum = parseFloat(total_price) + parseFloat(J.delivery_price);						
					}else{
						tsum = parseFloat(total_price)
					}
				}else{	
					tsum = parseFloat(total_price)
				}
				var ttotal_price = tsum;
				w.total = parseFloat(ttotal_price*quantitysec).toFixed(Main.IS_DECIMAL_POINT);	
				...
				if(J.psettingval==1){
					J.delivery_price=Main.NullToEmpty(J.delivery_price);
					if(Main.deliveryType == 'delivery' && J.delivery_price !=''){
						w.total = parseFloat(w.total) + parseFloat(J.delivery_price);
					}	
				}	
			}
	...
	GetCartItemsCount: function (){	
		...
		for (i in Shopping.Cart.business){
			...
			if(IS_SCRIPTID != 6){
				...
				{			
					...
					if (e >= Shopping.Cart.business[i].minimum || Main.excheck == 'excheck'){
				...
				if(Main.businessSetting.checkout_popup_settings == 0)
				{
					...
					if (Shopping.Cart.business[i].minimum != "" && Shopping.Cart.business[i].minimum != 0){
					if (e >= Shopping.Cart.business[i].minimum || Main.excheck == 'excheck'){
					var mor = '<center><input type="button" class="pop-up-read" onclick="Shopping.OpenCartGuest()" value="<?= $lang_resource['MOBILE_CHECKOUT_ORDER_NOW'] ?>"></center>'
					$("#min_order").empty().append(mor);
					}else{
					var mor ='<p class="check-out"><?= $lang_resource['REACH_MIN_ORD'] ?> '+Shopping.ActiveBusinesscurrency+' '+Shopping.Business[ai].minimum+' <?= $lang_resource['REACH_MIN_ORD1'] ?></p>';
					mor = '<center><input type="button" class="pop-up-read" value="<?= $lang_resource['SHOPPING_FOURTH_MINIMUM_VALUE'] ?> '+Shopping.Business[0].currency+' '+Shopping.Cart.business[i].minimum+'"></center>'
					$("#min_order").empty().append(mor);
					}
					}else{
					if(e > 0)
					$(".btn-red").removeClass("order_now_btn");
					}
					/*======================= //2mobile cart popup =============================================*/
				}
				}else{
					...
					if (Shopping.Cart.business[i].minimum != "" && Shopping.Cart.business[i].minimum != 0){
						if (e >= Shopping.Cart.business[i].minimum || Main.excheck == 'excheck'){
							if(Main.WhereAmIData.reservestatus == 'reservation') {
								var mor = '<button type="button" class="order_now_btn checkout_btn" onclick="Shopping.OpenCartCheck(true)"><?= $lang_resource['SHOPPING_FOURTH_ORDER_NOW_RESERVE'] ?></button>'
							}else{
								var mor = '<button type="button" class="order_now_btn checkout_btn" onclick="Shopping.OpenCartCheck()"><?= $lang_resource['SHOPPING_FOURTH_CHECK_OUT'] ?></button>'
							}
							$("#min_order").empty().append(mor);
						}else{
							var mor = '<button type="button" class="order_now_btn checkout_btn order_now_btn_gray" ><?= $lang_resource['SHOPPING_FOURTH_MINIMUM_VALUE'] ?>'+Shopping.Business[0].currency+' '+Shopping.Cart.business[i].minimum+'</button>'
							$("#min_order").empty().append(mor);
						}
					}else{
						if(e > 0)
				...
				if (Shopping.Cart.business[i].minimum != "" && Shopping.Cart.business[i].minimum != 0){
					if (e >= Shopping.Cart.business[i].minimum || Main.excheck == 'excheck'){
						var mor ='<div class="cartbuy_hd" onclick="RestMenuList.OpenCheckout()"><?=$lang_resource['SHOPPING_FOURTH_CHECK_OUT']?></div>'
					}else{
		...
		if(Shopping.Cart.business) {
			...
			if(viewDevice == "Mobile") {
				$("#itemCount").html("("+qn+")");
				$("#pop_up_item_num1").html(qn);
				$("#pop_up_item_num2").html(qn);
				$("#itemCount4").html(qn);
			}
		}else{
    ...
    UpdateTotals: function ()
    {
		...
		{
			var w = Main.GetIndexOnPropertyValueFound(Shopping.Business, "id", Shopping.ActiveBusiness);
			Main.servicefee = Shopping.Business[w].servicefee
			
			/*if((Main.servicefee=='') || (typeof Main.servicefee=='undefined')){
			Main.servicefee=0;
			}*/
			if(Main.servicefee>0){
			Shopping.Cart.servicefeeTotal=parseFloat((parseFloat(Main.servicefee)*parseFloat(d))/100);
			Shopping.Cart.servicefeeTotal1=(parseFloat((parseFloat(Main.servicefee)*parseFloat(d))/100)).toFixed(Main.IS_DECIMAL_POINT)
		}
		...
		if(Shopping.Cart.totalfinla)
			{
				...
				$("#availablepoints").html(' <span>'+Shopping.Cart.AvailablePointsAftr.toFixed(Main.IS_DECIMAL_POINT)+'</span>');
    ...
    UpdateCartTotals: function (){
		...
		Main.avaQtyCount=Array();
		for (var f in Shopping.Cart.business){
			for (var e in Shopping.Cart.business[f].dishes){
				if(Main.businessSetting.available_product == 1 && IS_SCRIPTID == 0 && ISIFRAME == 0){
					var qval = document.getElementById(Shopping.Cart.business[f].dishes[e].id).value;	
					if ($.inArray(Main.avaQtyCount.id, Shopping.Cart.business[f].dishes[e].id) == -1) {	
						Main.avaQtyCount.push({ 
							"id" : Shopping.Cart.business[f].dishes[e].id,
							"value" : qval				
						});
					}
				}

				if(Shopping.Cart.business[f].dishes[e].total){
					d = parseFloat(parseFloat(d) + parseFloat(Shopping.Cart.business[f].dishes[e].total)).toFixed(Main.IS_DECIMAL_POINT)
				}
				if(Shopping.Cart.business[f].dishes[e].expressService){
					var d = 0;
					d = parseFloat(parseFloat(d) + parseFloat(Shopping.Cart.business[f].dishes[e].expressService)).toFixed(Main.IS_DECIMAL_POINT);
				}
		...
		//alert(Shopping.Business[w].servicefee)
		...
		$('#pop_up_item_num1').html(qn);
		$('#pop_up_item_tot1').html(Shopping.Business[0].currency+' '+Shopping.Cart.total);
		$('#pop_up_item_num2').html(qn);
		$('#pop_up_item_tot2').html(Shopping.Business[0].currency+' '+Shopping.Cart.total);

		if(Main.excheck == 'excheck'){	
			Checkout.OrderDetailsPage();			
		}
	...
	OpenCartCheck: function(chkOrdrNReserv){
		...
		if (Shopping.Cart.business.length == 0){
			...
			swal("Error","<?= $lang_resource['SHOPPING_PAYMENT_NOT_SELECTED'] ?>","error");
			return
		}else if(chkOrdrNReserv){
			if(globalReserve.Free.length == 0 && globalReserve.Table.length == 0 && globalReserve.Room.length == 0){
				swal("Error","<?= $lang_resource['SHOPPING_RESERVE_AND_ORDER_CHECK'] ?>","error");
				return
   ...
   OpenCart: function(){
    ...
    UserUpdate: function (f, d)
    {
		...
		case "braintreeexpiryYY":
            Shopping.Cart.buyer.braintreeexpiry = f.value;
            break;	
			
		case "braintreeexpiryMM":
            Shopping.Cart.buyer.braintreeexpiry = f.value;
            break;		
    ...
    RemoveFromCart: function (c, d, g){

		var F = Main.GetIndexOnPropertyValueFound(Shopping.Menu.dishes, "id", g);	
		if(Main.businessSetting.available_product == 1 && IS_SCRIPTID == 0 && ISIFRAME == 0){
			var oldval = document.getElementById(g).value;
			var stockQty = Shopping.Menu.dishes[F].stock_qty;
			document.getElementById(g).value = parseInt(oldval) - 1;			
			Main.countQty = document.getElementById(g).value;
		}
		...
		if(quantity==1){
			...
			if(viewDevice == "Mobile") {
				if(GlobalPagecheck == "2"){
					...
					if(Main.businessSetting.checkout_popup_settings == 0){
						Checkout.OrderDetailsPage_new(true); //for mobile cart popup version only
					}
				}
				//return false;
			}
			...
			if (Shopping.Cart.business.length != 0) {
				for (var i = 0;i <= lastid;i++){
					...
					if(Main.businessSetting.checkout_popup_settings == 0){
						n += RestMenuList.MenuCartlist_new(i)	 //for mobile cart popup only		
					}
				}
				del = 1;
			}
			...
			if(viewDevice == "Mobile") {
				if(GlobalPagecheck == "2"){
					...
					if(Main.businessSetting.checkout_popup_settings == 0){							
						Checkout.OrderDetailsPage_new(true); //for mobile version cart popup only
					}
					return false;
				}
			}
		...
		for (i in Shopping.Cart.business){
			...
			if(viewDevice == "Mobile") {	
				...
				if (Shopping.Cart.business[i].minimum != "" && Shopping.Cart.business[i].minimum != 0){
					if (e >= Shopping.Cart.business[i].minimum){
						if(Main.WhereAmIData.reservestatus == 'reservation') {
							var mor = '<button type="button" class="checkout_btn" onclick="Shopping.OpenCartCheck(true)"><?= $lang_resource['SHOPPING_FOURTH_ORDER_NOW_RESERVE'] ?></button>'
						}else{
							var mor = '<button type="button" class="checkout_btn" onclick="Shopping.OpenCartCheck()"><?= $lang_resource['SHOPPING_FOURTH_CHECK_OUT'] ?></button>'
						}
						$("#min_order").empty().append(mor);
					}else{
						var mor = '<button type="button" class="checkout_btn order_now_btn_gray" ><?= $lang_resource['SHOPPING_FOURTH_MINIMUM_VALUE'] ?>'+Shopping.Business[0].currency+' '+Shopping.Cart.business[i].minimum+'</button>'
						$("#min_order").empty().append(mor);
					}
				}else{
					if(e > 0)
						$(".checkout_btn").removeClass("order_now_btn_gray");
				}				 
			}
			<!--MINIMUM PURCHASE-->
		}
		...
		if(Shopping.Cart.business) {
			...
			if(viewDevice == "Mobile") {
				$("#itemCount").html("("+0+")");
				$("#pop_up_item_num1").html(0);
				$("#pop_up_item_num2").html(0);
				$("#itemCount4").html(0);
			}	
		}
	},
		...
		if(Main.panelsetting == "2"){
			$.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchLatestDeliveryZone","id":' + Shopping.ActiveBusiness+'}]', function (b){
				...
				if((Shopping.FormatPriceNotFree(Main.WhereAmILocationData.price1) != 0.000) || (Shopping.FormatPriceNotFree(Main.WhereAmILocationData.price2) != 0.000) || (Shopping.FormatPriceNotFree(Main.WhereAmILocationData.price3) != 0.000)){				
					rec+='<?= $lang_resource['DELIVERYP_V21'] ?> '
				}

				if(Shopping.FormatPriceNotFree(Main.WhereAmILocationData.price1) != 0.000){
					rec +='<img src="panel/<?=$moduleName?>/images/green-circle.png" alt="">'+Main.car+' '+Shopping.FormatPriceNotFree(Main.WhereAmILocationData.price1);
				}
				
				if(Shopping.FormatPriceNotFree(Main.WhereAmILocationData.price2) != 0.000){
					rec +='<img src="panel/<?=$moduleName?>/images/red-circle.png" alt="">'+Main.car+' '+Shopping.FormatPriceNotFree(Main.WhereAmILocationData.price2)
				}
				
				if(Shopping.FormatPriceNotFree(Main.WhereAmILocationData.price3) != 0.000){
					rec+=' <img src="panel/<?=$moduleName?>/images/red-circle.png" alt="">'+Main.car+' '+Shopping.FormatPriceNotFree(Main.WhereAmILocationData.price3)
				}
	...
	//	alert(1)
		//alert(JSON.stringify(Main.WhereAmILocationData));
		        e= new Object();
    ...
    PlaceOrder: function (paypalid)
    {
        ...
        else
		{
			...
			if(setpayment==false){
				swal("Error","<?= $lang_resource['BUSINESS_PAYMENT_VALIDATION'] ?>","error");
				return
			}
		...
		Shopping.CartExtra = new Object();
		Shopping.CartExtra = Shopping.Cart.buyer
       ...
       // this.SavingOrder = true;	
		...
		//	Shopping.Cart.buyer['city'] = cityid;
            ...
            if (f != "") {
                ...
                Shopping.businesscart = new Array();
                Shopping.businesscart = Shopping.Cart.business
                Shopping.EmptyCart();
		...
		if(IS_REVIEW_ENABLED == 1 || IS_PAYPAL_ENABLED == 1){
			...
			for (var idx in Shopping.Cart.business){
				...
				for (var es in Shopping.Cart.business[idx].dishes){
					if(Shopping.Cart.business[idx].dishes[es].name)
...
if(globalReserve.Free.length != 0 || globalReserve.Table.length != 0 || globalReserve.Room.length != 0){
		...
		if(Main.checkoutinfo['Takeout Date'].status == 't' && Main.checkoutinfo['Takeout Date'].required == 't'){
			...
			if(Main.checkoutinfo['Name'].status == 't' && Main.checkoutinfo['Name'].required == 't' ){				
				if (document.getElementById("buyername").value ==""){
					...
					Shopping.Cart.buyer.name = document.getElementById("buyername").value
				}
			}
			...
			if(Main.checkoutinfo['Last Name'].status == 't' && Main.checkoutinfo['Last Name'].required == 't'){
				if (document.getElementById("buyerlastname").value ==""){
					...
					Shopping.Cart.buyer.lastname2 = document.getElementById("buyerlastname").value
				}
			}
			...
			if(Main.checkoutinfo['Email'].status == 't' && Main.checkoutinfo['Email'].required == 't'){
				if (document.getElementById("buyeremail").value ==""){
					...
					Shopping.Cart.buyer.email = document.getElementById("buyeremail").value
				}				
			}
			...
			if(Main.checkoutinfo['Full Address'].status == 't' && Main.checkoutinfo['Full Address'].required == 't'){
				if (document.getElementById("buyeraddress").value ==""){
					...
					Shopping.Cart.buyer.address = document.getElementById("buyeraddress").value
				}
			}
			Shopping.Cart.buyer.address= Shopping.Cart.buyer.address.replace("%20", " ");
			...
			if(Main.checkoutinfo['APT/Suit'].status == 't' && Main.checkoutinfo['APT/Suit'].required == 't'){
				if (document.getElementById("buyerapi").value ==""){
					alert("<?= $lang_resource['SHOPPING_FOURTH_ERROR_APT_SUIT'] ?>");
					return false
				}else{
					Shopping.Cart.buyer.api = document.getElementById("buyerapi").value
				}
			}

			if(Main.checkoutinfo['Zipcode'].status == 't' && Main.checkoutinfo['Zipcode'].required == 't'){
				if (document.getElementById("buyerzipcode").value ==""){
					alert("<?= $lang_resource['SHOPPING_FOURTH_ERROR_ZIP_CODE'] ?>");
					return false
				}else{
					Shopping.Cart.buyer.zipcode = document.getElementById("buyerzipcode").value
				}
			}
			...
			if(Main.checkoutinfo['City'].status == 't' && Main.checkoutinfo['City'].required == 't'){
				if (document.getElementById("buyercity").value ==""){
					...
					Shopping.Cart.buyer.city = document.getElementById("buyercity").value
				}
			}
			...
			if(Main.checkoutinfo['Area / Neighborhood'].status == 't' && Main.checkoutinfo['Area / Neighborhood'].required == 't'){
				if (document.getElementById("buyercolony").value ==""){
					...
					Shopping.Cart.buyer.colony = document.getElementById("buyercolony").value
				}
			}

			if(Main.checkoutinfo['Where did you find about us'].status == 't' && Main.checkoutinfo['Where did you find about us'].required == 't'){
				if (document.getElementById("buyerreference").value ==""){
					alert("<?= $lang_resource['SHOPPING_FOURTH_ERROR_REFERENCE'] ?>");
					return false
				}else{
					Shopping.Cart.buyer.reference = document.getElementById("buyerreference").value
				}
			}
			...
			if(Main.checkoutinfo['Phone'].status == 't' && Main.checkoutinfo['Phone'].required == 't'){
				if (document.getElementById("buyertel").value ==""){
					...
					Shopping.Cart.buyer.tel = document.getElementById("buyertel").value
				}
			}
			...
			if(Main.checkoutinfo['Tip For The Driver'].status == 't' && Main.checkoutinfo['Tip For The Driver'].required == 't' && Main.deliveryType!='pickup'){
				if (document.getElementById("buyertips").value ==""){
					alert("<?= $lang_resource['SHOPPING_FOURTH_ERROR_TIPS'] ?>");
					return false
				}else{
					Shopping.Cart.buyer.tips = document.getElementById("buyertips").value
				}
			}

			if(Main.checkoutinfo['Discount Coupon'].status == 't' && Main.checkoutinfo['Discount Coupon'].required == 't'){
				if (document.getElementById("discountcoupon").value == "" || !document.getElementById("discountcoupon")){
					alert("<?= $lang_resource['SHOPPING_FOURTH_ERROR_DISCOUNT'] ?>");
					return false
				}
			}
			...
			if (document.getElementById('paymethod-authorize-check') && authorizednet.checked){
				if (Shopping.Cart.buyer.firstname == "" || !Shopping.Cart.buyer.firstname){
					swal("Error","<?= $lang_resource['CONTROL_PANEL_AUTHORIZE_FIRSTNAME_ALERT'] ?>","error");
					return false;
				}
				if (Shopping.Cart.buyer.lastname == "" || !Shopping.Cart.buyer.lastname){
					swal("Error","<?= $lang_resource['CONTROL_PANEL_AUTHORIZE_LASTNAME_ALERT'] ?>","error");
					return false;
				}
				...
				if (Shopping.Cart.buyer.cityname == "" || !Shopping.Cart.buyer.cityname){
					swal("Error","<?= $lang_resource['CONTROL_PANEL_AUTHORIZE_CITYNAME_ALERT'] ?>","error");
					return false;
				}
				if (Shopping.Cart.buyer.state == "" || !Shopping.Cart.buyer.state){
					swal("Error","<?= $lang_resource['CONTROL_PANEL_AUTHORIZE_STATE_ALERT'] ?>","error");
					return false;
				}
				if (Shopping.Cart.buyer.countryname == "" || !Shopping.Cart.buyer.countryname){
					swal("Error","<?= $lang_resource['CONTROL_PANEL_AUTHORIZE_COUNTRYNAME_ALERT'] ?>","error");
					return false;
				}
				if (Shopping.Cart.buyer.zip == "" || !Shopping.Cart.buyer.zip){
					swal("Error","<?= $lang_resource['CONTROL_PANEL_AUTHORIZE_ZIP_ALERT'] ?>","error");
					return false;
				}
			...
			if (document.getElementById('paymethod-mercury-check')){
				...
				if (document.getElementById('paymethod-mercury-check') && mercurypayment.checked){
					if (Shopping.Cart.buyer.mercury_acno == "" || !Shopping.Cart.buyer.mercury_acno){
						swal("Error","<?= $lang_resource['CONTROL_PANEL_MERCURY_ACNO_ALERT'] ?>","error");
						return false;
					}
					if (Shopping.Cart.buyer.mercury_exmm == "" || !Shopping.Cart.buyer.mercury_exmm){
						swal("Error","<?= $lang_resource['CONTROL_PANEL_MERCURY_EXMM_ALERT'] ?>","error");
						return false;
					}
					if (Shopping.Cart.buyer.mercury_exyy == "" || !Shopping.Cart.buyer.mercury_exyy){
						swal("Error","<?= $lang_resource['CONTROL_PANEL_MERCURY_EXYY_ALERT'] ?>","error");
						return false;
					}
				}
			}
			...
			if(Main.businessSetting.available_product == 1 && IS_SCRIPTID == 0){
				var sl = Main.avaQtyCount;
				Main.out = [];

				for (var i = 0, l = sl.length; i < l; i++) {
					var unique = true;
					for (var j = 0, k = Main.out.length; j < k; j++) {
						if ((sl[i].id === Main.out[j].id) && (sl[i].value === Main.out[j].value)) {
							unique = false;
						}
					}
					if (unique) {
				...
				Shopping.Cart.StockQty=Main.out;
			}
			return true;
		}
	},
	...
	PlaceOrderMacro: function (paypalid,paymentgetway)
    {
		...
		var I = new Object();
				...
				if(paymentgetway=="global"){
				I.paymethod.global = true;
				}
				if(paymentgetway=="btrans"){
				I.paymethod.btrans = true;
				}
				if(paymentgetway=="bsa"){
				I.paymethod.bsa = true;
				}
				if(paymentgetway=="azul"){
				I.paymethod.azul = true;
				}
				if(paymentgetway=="quickpay"){
				I.paymethod.quickpay = true;
				}
				if(paymentgetway=="paynl"){
				I.paymethod.paynl = true;
				}
				if(paymentgetway=="zaakpay"){
				I.paymethod.zaakpay = true;
				}
	   ...
	   Shopping.Cart.buyer.city = Main.GetPropertyValueOnPropertyValueFound(Shopping.Business, "id", Shopping.ActiveBusiness, "city");
        ...
        {	
           ...
           // $("#checkoutloading").hide();
			...
			if(paymentgetway=="maksekeskus"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/maksekeskus/index.php?id="+f;
			}
			
			if(paymentgetway=="skrill"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/skrill/index.php?id="+f;
			}
			
			
			if(paymentgetway=="voguepay"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/voguepay/voguepay.php?id="+f;
			}
			
			if(paymentgetway=="pexpress"){
				
				
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/pxpay/pxpay.php?id="+f;
			}
			
			
			if(paymentgetway=="transactium"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/transactium/ShowHPS.php?id="+f;
			}
		
			
			if(paymentgetway=="mercury"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/mercury/ProcessTransaction.php?id="+f;
			}
			
		
			if(paymentgetway=="paypaladaptive"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/paypaladaptive/samples/SimpleSamples/ParallelPay.php?id="+f;
			}
			if(paymentgetway == "mercadopago"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/mercadopago/mercadopago_ipncustom.php?id="+f;
			}

			if(paymentgetway=="payeezy"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/payeezy/index.php?id="+f;
			}
          if(paymentgetway=="payu"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/payu/index.php?id="+f;
			}
			if(paymentgetway=="stripe"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/stripe/index.php?id="+f;
			}

			if(paymentgetway=="paypalpro"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/paypalpro/index.php?id="+f;
			}
			if(paymentgetway=="paygistix"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/paygistix/index.php?id="+f;
			}
			if(paymentgetway=="global"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/global/index.php?id="+f;
			}
			if(paymentgetway=="btrans"){
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/btrans/index.php?id="+f;
			}
			if(paymentgetway=="bsa"){
				window.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/bsa/index.php?id="+f;
			}
			if(paymentgetway=="azul"){
				window.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/azul/index.php?id="+f;
			}
			if(paymentgetway=="quickpay"){
				window.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/quickpay/index.php?id="+f;
			}
			if(paymentgetway=="paynl"){
				window.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/paynl/index.php?id="+f;
			}
			if(paymentgetway=="zaakpay"){
				window.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/zaakpay/index.php?id="+f;
			}
	   ...
	   Shopping.Cart.buyer.city = Main.GetPropertyValueOnPropertyValueFound(Shopping.Business, "id", Shopping.ActiveBusiness, "city");
        ...
        $.post("panel/lib/front-main.php", "f=PlaceOrderbefore&data=" + JSON.stringify(Shopping.Cart)+paypalInfo, function (f)
        {
           ...
           // $("#checkoutloading").hide();
				...
				parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/authorizenet/authorizenet.php?id="+f;
       ...
       // $("#checkoutloading").show();
        ...
        $.post("panel/lib/front-main.php", "f=PlaceOrderbefore&data=" + JSON.stringify(Shopping.Cart)+paypalInfo, function (f)
        {
            ...
            if (b != Main.Aid)
            {
             ...
             Main.Ready();
            //$("#checkoutloading").hide();
			parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/worldpay/marchent.php?id="+f;
...
if(Shopping.Cart.buyer.city == "" || Main.NullToEmpty(Shopping.Cart.buyer.city) == "" ) {
       ...
       //$("#checkoutloading").show();
        ...
        $.post("panel/lib/front-main.php", "f=PlaceOrderbrainTree&data=" + JSON.stringify(Shopping.Cart)+paypalInfo, function (f)
        {
			...
			parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/braintreepayments/braintree_ipncustom.php?iid="+f;
...
if(Shopping.Cart.buyer.city == "" || Main.NullToEmpty(Shopping.Cart.buyer.city) == "" ) {
	   Shopping.Cart.buyer.city = Main.GetPropertyValueOnPropertyValueFound(Shopping.Business, "id", Shopping.ActiveBusiness, "city");
		...
		$.post("panel/lib/front-main.php", "f=PlaceOrderbefore&data=" + JSON.stringify(Shopping.Cart)+paypalInfo, function (f){
           ...
           // $("#checkoutloading").hide();
			...
			parent.location.href = "<?=$configRec['siteurl']?>/panel/payment-gateway/paypal/paypal_custom.php?id="+f;
	...
	SearchSaveReserveTime: function(){
		...
		Shopping.Cart.buyer.deliveryType = 'Pickup'
		reservetab = 1;
		var globalReserve = new Object();
		globalReserve.Room = new Array();
		globalReserve.Table = new Array();
	...
	ReserveNow: function (paypalid){ 
		if(Shopping.Cart.business!=''){
			if((Shopping.Cart.business[0].dishes) !=null){
				swal("Error","<?= $lang_resource['SHOPPING_SELECT_RESERVE_ORDER_ALERT'] ?>","error");
				return	
			}
		}
		...
		for (var x in Shopping.Business){
			...
			for (var y in t){
				for (var z in Shopping.CategoriesCustom){
					var arr = Shopping.CategoriesCustom[z].ids
					arr = arr.split(",")	
					if (arr.indexOf(t[y])!=-1){
			...
			if(fav){
				...
				if (C){
					...
					if (Shopping.Business[x].name.toLowerCase().indexOf(v) >= 0){
						...
						for (y in B){
							if (Main.GetPropertyValueOnPropertyValueFoundForIds(Shopping.CategoriesCustom, "ids", B[y], "name").toLowerCase().indexOf(v) >= 0){
								s.push(Shopping.Business[x]);
								break
							}
						}
