...
function initialize1() {
  ...
  if(document.getElementById('address').value !=""){
	...
	.done(function(data){	
	 ...
	 var marker = new google.maps.Marker({
				position: new google.maps.LatLng(Main.lat, Main.long),
				title: 'Marker',
				map: map,
				draggable: false
			});   
  ...
  google.maps.event.addListener(autocomplete, 'place_changed', function() {
	...
	e = new Object();
	e.latitud = place.geometry.location.lat();
	e.longitud = place.geometry.location.lng();
	if(place.address_components[0].types=="postal_code"){
	e.zipcode = place.address_components[0].long_name;
	}
	e.zoom = 15;
	Main.lat = place.geometry.location.lat();
	Main.long = place.geometry.location.lng();
	
	
	Forms.UpdateValue("whereami", "location", JSON.stringify(e), true);
//console.log(e);
...
//alert(GoogleMap.distance1);
        hh.push('<div style="float: left; color:#000; overflow: hidden; height: 130px; font-size: 11px; padding:3px; width: 270px; margin-top: 15px;" onclick="Shopping.OpenBusiness('+businessid[i]+')">'+rvw+'<small>('+rating+')</small><hr style="margin:10px 0px 0px 0px;" /><div style="height: 32px;"><div style="width: 15%;float: left;height: 32px;"><img src="'+titleimage+'"></div><h3 class="hand" style="float:left;width:85% ;line-height:10px; margin:10px 0px 10px 0px;"  onclick="Shopping.OpenBusiness('+businessid[i]+')" >'+title[i]+'</h3></div><p style="margin:0px !important">'+street[i]+',&nbsp&nbsp&nbsp'+colony[i]+'</p>'+dpricetext+'<p style="margin: 2px;"><?=$lang_resource['DISTANCE']?> '+ distanceInKM +' '+distanceformat+'</p><div class="payment-method-icon" style="float:left; margin: -5px 5px;"><span class="" style="float:left;margin-left: -3px;margin-right: 6px;margin-top: 8px;"><?=$lang_resource['TEMPLATE_PAYMENTS']?> </span>'+carddetails+'</div></div>');
        ...
        if(!Main.RedirectToCategory) {
         var m = new google.maps.Marker({
                ...
                html:      '<h3 style="color:#000;"><?= $lang_resource['FRONT_FORM_YOU_ARE_HERE'] ?></h3>'
            });
            google.maps.event.addListener(m, 'click', function() {
                info_window.setContent(this.html);
                info_window.open(google_map, this);
