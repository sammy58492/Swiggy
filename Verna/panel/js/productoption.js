...
var ProductOption = {
	 ...
	 add_product_options_db:function(cart,id,stock_qty,options,comments,optionsid,total_price){
	 	if(Main.businessSetting.available_product == 1 && IS_SCRIPTID == 0){
		Main.stock_qty1 = stock_qty;
		Main.stock_id = id; 	
		if(Main.stock_qty1 > 0)
		{
			var stock_id_val = document.getElementById(Main.stock_id).value;
			if(stock_id_val == Main.stock_qty1)
			{
				...
				var  extrasAll = JSON.parse(Shopping.Menu.dishes[F].extras);
				   ...
				   if(ingredientsAlls.length!= 0 && extrasAll.length == 0) {
					...
					Shopping.AddToCart(cart,id,options,comments,optionsid,total_price)
						...
						$("#dish_"+id+"_imghide").html('<button type="button" class="add_plus" id="dish_' + id + '_imglink" >+</button>');	 
	...
	add_only_ingredient:function(cart,id,options,comments,optionsid,total_price){
		...
		e +='<button type="button" class="close" data-dismiss="modal" onclick="Popup.Close()" aria-label="Close"><span aria-hidden="true">&times;</span></button>'
		e +='<h4 class="modal-title" id="myModalLabel">'+Main.TitleCase(productDetail.name)+' '+Shopping.Business[curIdex].currency+''+productDetail.price+'</h4>'
		e +='<hr>'
		e +='<small>'+productDetail.description+'</small>'
		e +='</div>'
		e +='<div class="modal-body">'
		e +='<div class="row">'
		e +='<div class="col-md-12">'
		e +='<div class="form-group">'
		e +='<label style="font-weight:400"><?= $lang_resource['PRODUCT_POTIONS_QUANTITY'] ?> :</label>'
		e +='<select class="form-control" onchange="ProductOption.updateOptionsquantity(this)">'
		for(var s=1; s<21; s++) { 
		...
		if(Main.NullToEmpty(Shopping.Menu.dishes[F].ingredients)!=""){
			e +='<div class="row">'
			e +='<div class="col-md-12">'
			e +='<h5 class="p_option_heading"><?=$lang_resource['PRODUCT_POTIONS_INGREDIENTS_TEXT']?></h5>'
			e +='<div class="option_box">'

			var counter = 0;
			var counter_length = ingredientsAll.length
			for(var ing in ingredientsAll ){
				...
				e +='<div class="small_ck_dv">'
				e +='<input type="checkbox" name="01" id="' +"ing_"+ingredientsAll[ing] + '" value="'+ingredientsAll[ing]+'" onchange="ProductOptionsForms.CheckTextInputForIngredient(' + Main.Quote(ingredientsAll[ing]) + ',this);" checked="checked" >'
				e +='<label for="' +"ing_"+ingredientsAll[ing] + '">'+ingredientsAll[ing]+'</label>'
				e +='</div>'<!--radio_dv-->
				e +='</div>'<!--col-md-6-->

				counter++;
				if(counter % 2 == 0 || counter_length == counter){
					e +='</div>'<!--row-->
				}
			}	
		...
		e +='<div class="row">'
		e +='<div class="col-md-12">'
		e +='<h5 class="p_option_heading"><?= $lang_resource['PRODUCT_POTIONS_SPECIAL_INSTRCTION'] ?></h5>'
		e +='<textarea class="form-control" placeholder="<?= $lang_resource['PRODUCT_POTIONS_ADD_SPECIAL_INSTRCTION'] ?>" onkeyup="ProductOption.optionCommentUpdate(this)" style="height:56px; font-size:12px;"></textarea>'
		e +='</div>'<!--col-md-12-->
		e +='</div>'<!--row-->
		...
		e +='<button type="button" class="popup_btn" style=" width:200px;" onclick="ProductOption.SaveEndUserChoices();"><?= $lang_resource['PRODUCT_POTIONS_ADD_CART'] ?></button>'
		e +='</div>'
		...
		$("#dish_"+id+"_imghide").html('<button type="button" class="add_plus" id="dish_' + id + '_imglink" onclick="ProductOption.add_product_options_db(this,' + id + ')">+</button>');
	...
	edit_only_ingredient:function(id,edit_id){
		...
		if(Main.NullToEmpty(Shopping.Menu.dishes[F].ingredients)!="") {
			...
			for(var ing in ingredientsAll){				
				...
				if(ProductOption.in_array(ingredientsAll[ing],arr_opption)) {
					...
					e +='<div class="col-md-6">'
					e +='<div class="small_ck_dv">'
					e +='<input type="checkbox" name="01" id="' +"ing_"+ingredientsAll[ing] + '" value="'+ingredientsAll[ing]+'" onchange="ProductOptionsForms.CheckTextInputForIngredient(' + Main.Quote(ingredientsAll[ing]) + ',this);" >'
					e +='<label for="' +"ing_"+ingredientsAll[ing] + '">'+ingredientsAll[ing]+'</label>'
					e +='</div>'<!--radio_dv-->
					e +='</div>'<!--col-md-6-->
				}				
		...
		e +='<div class="col-md-12">'
		e +='<h5 class="p_option_heading"><?= $lang_resource['PRODUCT_POTIONS_SPECIAL_INSTRCTION'] ?></h5>'
		e +='<textarea class="form-control" placeholder="<?= $lang_resource['PRODUCT_POTIONS_ADD_SPECIAL_INSTRCTION'] ?>" onkeyup="ProductOption.optionCommentUpdate(this)" style="height:56px; font-size:12px;">'+Main.NullToEmpty(Shopping.Cart.business[0].dishes[edit_id].comments)+'</textarea>'
		e +='</div>'<!--col-md-12-->
		e +='</div>'<!--row-->
		...
		e +='<div class="modal-footer">'
		e +='<button type="button" class="popup_btn" style=" width:200px;" onclick="ProductOption.EditEndUserChoices();"><?= $lang_resource['PRODUCT_POTIONS_ADD_CART'] ?></button>'
		e +='</div>'
	...
	add_product_options:function(cart,id){
		...
		$.post("panel/lib/productoption.php", 'data=[{"operation":"GetAllSets","id":'+id+"}]", function (b){
			...
			e +='<h4 class="modal-title" id="myModalLabel">'+Main.TitleCase(productDetail.name)+' '+Shopping.Business[curIdex].currency+''+productDetail.price+'</h4>'
			e +='<hr>'
			e +='<small>'+Main.TitleCase(productDetail.description)+'</small>'
			e +='</div>'
			...
			e +='<div class="form-group">'
			e +='<label style="font-weight:400"><?= $lang_resource['PRODUCT_POTIONS_QUANTITY'] ?> :</label>'
			e +='<select class="form-control" id="qty_count" onchange="ProductOption.updateOptionsquantity(this)">'
			...
			if(data.review.extras_details[0].person == 1){
				...
				e +='<div class="col-md-12">'
				e +='<div class="form-group">'
				e +='<label style="font-weight:400"><?=$lang_resource['SHOPPING_NO_OF_PERSON']?> :</label>'			
				e += '<select id="person_count" class="form-control" onchange="ProductOption.updateOptionsperson(this)">'
			...
			if(Main.NullToEmpty(Shopping.Menu.dishes[F].ingredients)!="") {
				...
				ArHtml[0] = ''
				ArHtml[0] +='<div class="row">'
				ArHtml[0] +='<div class="col-md-12">'
				ArHtml[0] +='<h5 class="p_option_heading"><?=$lang_resource['PRODUCT_POTIONS_INGREDIENTS_TEXT']?></h5>'
				ArHtml[0] +='<div class="option_box">'
				...
				for(var ing in ingredientsAll ) {
					...
					ArHtml[0] +='<div class="col-md-6">'
					ArHtml[0] +='<div class="small_ck_dv">'
					ArHtml[0] +='<input type="checkbox" id="' +"ing_"+ingredientsAll[ing] + '" value="'+ingredientsAll[ing]+'" onchange="ProductOptionsForms.CheckTextInputForIngredient(' + Main.Quote(ingredientsAll[ing]) + ',this);" checked="checked" >' 
					ArHtml[0] +='<label for="' +"ing_"+ingredientsAll[ing] + '">'+ingredientsAll[ing]+'</label>'
					ArHtml[0] +='</div>'<!--radio_dv-->
					ArHtml[0] +='</div>'<!--col-md-6-->
				...
				ArHtml[0] +='</div>'<!--option_box-->
				ArHtml[0] +='</div>'<!--col-md-12-->
				ArHtml[0] +='</div>'<!--row-->
			}
						...
						$.each(data['review']['option_counts'],function(index,value){
							...
							$.post("panel/lib/productoption.php", 'data=[{"operation":"GetOptions","id":'+ temp +'}]', function (b){
								...
								if(b !=""){
									...
									ArHtml[cur_option] =''
									ArHtml[cur_option] +='<div class="row">'
									ArHtml[cur_option] +='<div class="col-md-12">'

									if(option_data['review']['extras_details'][0]['min_sel']>0) {
										ArHtml[cur_option] +='<h5 class="p_option_heading">'+option_data['review']['extras_details'][0]['text_to_end_user'] + ': (<?= $lang_resource['PRODUCT_POTIONS_ADD_REQUIRED'] ?>)</h5>'
									}else{
										ArHtml[cur_option] +='<h5 class="p_option_heading">'+option_data['review']['extras_details'][0]['text_to_end_user']+'</h5>'
									}									
									ArHtml[cur_option] +='<div class="option_box">'


									var counter1 = 0;
									var counter1_length = option_data['review']['extras_details'].length
									...
									$.each(option_data['review']['extras_details'],function(option_index,option_value){
										...
										if(option_value['max_sel']>1 && option_value['min_sel']>=0) {
											...
											ArHtml[cur_option] +='<div class="col-md-6">'
											ArHtml[cur_option] +='<div class="small_ck_dv sdsd">'

											ArHtml[cur_option] += ProductOptionsForms.CreateCheckBoxPropertyChoiceext("extras_details", option_value["choice_name"],false,'','',option_value['max_sel'],option_value['min_sel'],cname,option_value['text_to_end_user'],'',option_value['showoption'],option_value["choice_id"],option_value['set_id'])

											ArHtml[cur_option] +='<label for="'+dyid+'" >'+option_value['choice_name']+'</label><span class="pull-right">'+Shopping.ActiveBusinesscurrency+''+parseFloat(option_value['price']).toFixed(Main.IS_DECIMAL_POINT)+'</span>'


											ArHtml[cur_option] +='</div>'<!--radio_dv-->
											ArHtml[cur_option] +='</div>'<!--col-md-6-->

											counter1++;
											if(counter1 % 2 == 0 || counter1_length == counter1){
									...
									if(flag==1) {
										var counter2 = 0;								
										var counter2_length = option_data['review']['extras_details'].length
										$.each(option_data['review']['extras_details'],function(option_index,option_value){
											var cname = "dynamicname-"+option_value['option_id'];
											...
											ArHtml[cur_option] +='<div class="col-md-6">'
											ArHtml[cur_option] +='<div class="small_radio_dv">'

											ArHtml[cur_option] += ProductOptionsForms.CreateRadioPropertyChoice("extras_details", "choice_name_drop-"+option_value['choice_id'],"choice_name_drop"+value["option_id"],option_value['choice_id'], false,option_value['min_sel'],option_value['max_sel'],cname,option_value['text_to_end_user'],option_value['showoption'],option_value['set_id'])

											ArHtml[cur_option] +='<label for="'+"choice_name_drop-"+option_value['choice_id']+'" onclick="ProductOption.checkedval(\'' + idrs + '\',2)" >'+option_value['choice_name']+'</label><span class="pull-right">'+Shopping.ActiveBusinesscurrency+''+parseFloat(option_value['price']).toFixed(Main.IS_DECIMAL_POINT)+'</span>'
											...
											counter2++;
											if(counter2 % 2 == 0 || counter2_length == counter2){
												ArHtml[cur_option] +='</div>'<!--row-->
											}											
										});
									...
									ArHtml[cur_option] +='</div>'<!--option_box-->
									ArHtml[cur_option] +='</div>'<!--col-md-12-->
									ArHtml[cur_option] +='</div>'<!--row-->

									ArHtml[cur_option] +='<div id="'+showvalue+'" class="row"></div>'
									count_temp++; 
									count_temp_old++;
			...
			if(Main.NullToEmpty(Shopping.Menu.dishes[F].ingredients)!="") {
				...
				for(var ing in ingredientsAll ) {	
					...
					if(ProductOption.in_array(ingredientsAll[ing],arr_ingredients)) {
						...
						ArHtml[0] +='<input type="checkbox" id="' +"ing_"+ingredientsAll[ing] + '" value="'+ingredientsAll[ing]+'" onchange="ProductOptionsForms.CheckTextInputForIngredient(' + Main.Quote(ingredientsAll[ing]) + ',this);" >' 
						ArHtml[0] +='<label for="' +"ing_"+ingredientsAll[ing] + '">'+ingredientsAll[ing]+'</label>'
					}
				...
				$.post("panel/lib/productoption.php", 'data=[{"operation":"GetOptions","id":'+ temp +"}]", function (b){
					...
					$.each(option_data['review']['extras_details'],function(option_index,option_value){
						...
						if(option_value['max_sel']>1 && option_value['min_sel']>=0){
							...
							ArHtml[cur_option] += ProductOptionsForms.CreateCheckBoxPropertyChoiceext("extras_details", option_value["choice_name"],true,'','',option_value['max_sel'],option_value['min_sel'],cname,option_value['text_to_end_user'],optionChoiceId,option_value['showoption'],option_value["choice_id"],option_value['set_id'])
					...
					ArHtml[cur_option] +='<label for="'+"choice_name_drop-"+option_value['choice_id']+'" onclick="ProductOption.checkedval(\'' + idrs + '\',2)" >'+option_value['choice_name']+'<span class="pull-right">'+Shopping.ActiveBusinesscurrency+''+parseFloat(option_value['price']).toFixed(Main.IS_DECIMAL_POINT)+'</span></label>'
	...
	showcondition:function(b,fieldName,productOptions){
		...
		e +='<div class="col-md-12">'
		if(option_data['review']['extras_details'][0]['min_sel']>0) {
			e +='<h5 class="p_option_heading">'+option_data['review']['extras_details'][0]['text_to_end_user'] + ' (<?= $lang_resource['PRODUCT_POTIONS_ADD_REQUIRED'] ?>)</h5>'			
		}else{
			e +='<h5 class="p_option_heading">'+option_data['review']['extras_details'][0]['text_to_end_user']+'</h5>'
		}
		e +='<div class="option_box">'

		var counter3 = 0;
		var counter3_length = option_data['review']['extras_details'].length;
		...
		$.each(option_data['review']['extras_details'],function(option_index,option_value){
			...
			if(option_value['max_sel']>1 && option_value['min_sel']>=0){
				...
				e +='<div class="col-md-6">'
				e +='<div class="small_ck_dv">'
				e += ProductOptionsForms.CreateCheckBoxPropertyChoiceext("extras_details", option_value["choice_name"],true,'','',option_value['max_sel'],option_value['min_sel'],cname,option_value['text_to_end_user'],productOptionss,option_value['showoption'],option_value["choice_id"],option_value['set_id'],JSON.stringify(ConditionRecord))
				e +='<label for="'+dyid+'" >'+option_value['choice_name']+'</label><span class="pull-right">'+Shopping.ActiveBusinesscurrency+''+parseFloat(option_value['price']).toFixed(Main.IS_DECIMAL_POINT)+'</span>'
				e +='</div>'<!--radio_dv-->
				e +='</div>'<!--col-md-6-->
				
				counter3++
				if(counter3 % 2 == 0 || counter3_length == counter3){
					e +='</div>'<!--row-->
				}
		...
		if(flag==1){
			...
			$.each(option_data['review']['extras_details'],function(option_index,option_value){
				...
				e +='<div class="row">'			
				e +='<div class="col-md-6">'
				e +='<div class="small_radio_dv">'
				e += ProductOptionsForms.CreateRadioPropertyChoice("extras_details","choice_name_drop-"+option_value['choice_id'],"choice_name_drop"+option_value["option_id"],option_value['choice_id'], chkv,option_value['min_sel'],option_value['max_sel'],cname,option_value['text_to_end_user'],option_value['showoption'],option_value['set_id'],'',JSON.stringify(ConditionRecord))
				e +='<label for="'+"choice_name_drop-"+option_value['choice_id']+'" onclick="ProductOption.checkedval(\'' + idrs + '\',2)">'+option_value['choice_name']+'</label><span class="pull-right">'+Shopping.ActiveBusinesscurrency+''+parseFloat(option_value['price']).toFixed(Main.IS_DECIMAL_POINT)+'</span>'
				e +='</div>'<!--radio_dv-->
				e +='</div>'<!--col-md-6-->
				e +='</div>'<!--row-->
			});			
		...
		e +='<div id="'+showvalue+'" class="row" ></div>';  
...
SaveEndUserChoices: function(){
	...
	for(s in ProductOptionsForms.Form ) {
			...
			if(ProductOptionsForms.Form[s].counti<ProductOptionsForms.Form[s].mini) {
				swal("Error","<?=$lang_resource['PRODUCTOPTION_ADDTOCART_ERROR']?> "+ProductOptionsForms.Form[s].mini+' <?=$lang_resource['PRODUCTOPTION_ADDTOCART_ERROR_NEED_ADD']?> '+ProductOptionsForms.Form[s].text ,"error");
				return false ;	
			}
				else {
    ...
    EditEndUserChoices:function(){
		...
		$.post("panel/lib/productoption.php", 'data=[{"operation":"GetTotalPricesEdit","ids":'+JSON.stringify(ProductOptionsForms.Form.extras_details)+"}]", function (b){
			...
			console.log(JSON.stringify(Shopping.Menu.dishes))
			...
			{
			Checkout.open_cart_popup();//for mobile version popup
			}
			}
...
CalculatePointValue:function(){
			            	...
			            	if(b!="")
			            	{
			            		...
			            		if(totalpointwantuse <= Main.availablepoint)
			            		{
				            		...
				            		else
				            		{
					            		...
					            		$("#availablepoints").html(' <span>'+availablepointsaftr.toFixed(2)+'</span>');
					            		$("#pointsapplied").html(' <span>'+totalpointwantuse+'</span>');
					            		$("#totalorderprice").html(' <span>'+finalvalue.toFixed(2)+'</span>');
					            		Shopping.Cart.totalfinla=finalvalue.toFixed(2);
   ...
   ChangePrice:function(b,option_id,set_id){
    ...
    {
 $('#loader_dv').addClass( "addloader" );
   $("#dish_"+Shopping.Cart.business[0].dishes[i].id+"_imghide").html('<button type="button" class="add_plus" id="dish_' + Shopping.Cart.business[0].dishes[i].id + '_imglink" onclick="ProductOption.add_product_options_db(this,' + Shopping.Cart.business[0].dishes[i].id + ')">+</button>');
