	...
	add_product_options_db:function(cart,id,stock_qty,options,comments,optionsid,total_price){
		...
		$.post("panel/lib/productoption.php", 'data=[{"operation":"GetAllSets","id":'+id+"}]", function (b){
			...
			if(ingredientsAlls.length!= 0 && extrasAll.length == 0) {			
				...
				ProductOption.add_product_options(cart,id,stock_qty);
			}
		});
	},
...
SaveEndUserChoices: function(){
	...
	if(Main.businessSetting.available_product == 1 && IS_SCRIPTID == 0){
		...
		if(parseInt(oldval) < parseInt(stockQty))
		{
		...
		if(mychoice_data[d]) {
		   ...
		   if(viewDevice == "Mobile") {
			   ...
			   if(GlobalPagecheck == "3")
			{
				...
				{
					Checkout.OrderDetailsPage_new(true); //only for mobile cart popup
				}
			}
			   return false;
		   }
...
CalculatePointValue:function(){
 ...
 $('#loader_dv').addClass( "addloader" );
   $("#dish_"+Shopping.Cart.business[0].dishes[i].id+"_imghide").html('<a href="javascript:void(0)" id="dish_' + Shopping.Cart.business[0].dishes[i].id + '_imglink"  onclick="ProductOption.add_product_options_db(this,' + Shopping.Cart.business[0].dishes[i].id + ')"><img src="images/step3-menu-list/add-icon-big.png"></a>');
	...
	updateOptionsquantity : function(frm,id) {
		if(Main.businessSetting.available_product == 1 && IS_SCRIPTID == 0){
			var old_drop_val1 = document.getElementById(Main.stock_id).value;
			var new_dropdown_value = parseInt(old_drop_val1) + parseInt(frm.value);
			if(new_dropdown_value <= parseInt(Main.stock_qty1))
			{
				quantitysec = parseInt(frm.value);
				Main.newqtyval = new_dropdown_value;
			}
			else
			{
    ...
    add_product_options:function(cart,id,stock_qty){
	   if (Main.Busy) {
		   return false;
		   }
		 var storeOptions = new Array();
        ...
        $.post("panel/lib/productoption.php", 'data=[{"operation":"GetAllSets","id":'+id+"}]", function (b){
			...
			e += '<div class="cont"><p style="font-size:12px;">'+Main.TitleCase(productDetail.description)+'</p>';
			e += '<div class="allheader"><?= $lang_resource['PRODUCT_POTIONS_QUANTITY'] ?> :</div>';
			e += '<select name="" id="qty_count" class="selectbox" onchange="ProductOption.updateOptionsquantity(this,id)">';
					...
					$.each(option_data['review']['extras_details'],function(option_index,option_value){
								...
								//	alert("1x"+set_total_option_count+"-"+count_temp)
									...
									//alert("2x"+set_total_option_count+"-"+count_temp)
										...
										$('#loader_dv').removeClass( "addloader" );
										$("#dish_"+id+"_imghide").html('<a href="javascript:void(0)" id="dish_' + id + '_imglink"  onclick="ProductOption.add_product_options_db(this,' + id + ','+stock_qty+')"><img src="images/step3-menu-list/add-icon-big.png"></a>');
