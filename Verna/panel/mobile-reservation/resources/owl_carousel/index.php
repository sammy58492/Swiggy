...
{
pg_prepare($link,'sql','SELECT id,isimg,customslug,scriptid FROM w_business WHERE enabled=$1 AND is_popular=$2 AND scriptid=$3');
$result = pg_execute($link,'sql',array(TRUE,TRUE,$_SESSION['scriptid']));
}
else
{
pg_prepare($link,'sql','SELECT id,isimg,customslug,scriptid FROM w_business WHERE enabled=$1 AND scriptid=$2');
$result = pg_execute($link,'sql',array(TRUE,$_SESSION['scriptid']));
}
