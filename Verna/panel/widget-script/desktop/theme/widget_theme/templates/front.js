  ...
  if(isMobile.any())
{
    ...
    {
		//var isResponsive = document.getElementById("isResponsive").value;
		 ...
		 if (Link.indexOf("oos_re_direct_to_business_") >= 0)
        {
			...
			var blname = document.getElementById("blistName").value;
			Shopping.ActiveBusinessSlugName = blname;
...
var Main = {
    ...
    InitInterface: function (){
       ...
       if (Main.RedirectToBusiness){
			Main.settingfront =  new Object();
			Main.businessSetting =  new Object();
			Main.settingfront.tab_delivery ='t';
			Main.settingfront.tab_pickup ='f';
			Main.settingfront.tab_reservation ='f';
			Main.businessSetting.businesspageheadersetting ='f';
		Shopping.BusinessCustomeSearch(Shopping.ActiveBusiness)
		}else{
        this.LoginBox();
		}
        Tags.Init()
	...
	placeRequestCollection:function(){
		...
		if ( Main.requestCollction.customer_name  == ""){
			alert("<?= $lang_resource['CUSTOMER_NAME_VV1'] ?>");
			return
		}
		if ( Main.requestCollction.customer_address1  == ""){
			alert("<?= $lang_resource['CUSTOMER_ADDRESS1_VV1'] ?>");
			return
		}
		if ( Main.requestCollction.customer_town  == ""){
			alert("<?= $lang_resource['CUSTOMER_TOWN_VV1'] ?>");
			return
		}
		if ( Main.requestCollction.customer_postcode  == ""){
			alert("<?= $lang_resource['CUSTOMER_POSTCODE_VV1'] ?>");
			return
		}
		if ( Main.requestCollction.customer_contactno  == ""){
			alert("<?= $lang_resource['CUSTOMER_CNO_VV1'] ?>");
			return
		}	
		if ( Main.requestCollction.resturent_name  == ""){
			alert("<?= $lang_resource['RESTURENT_NAME_VV1'] ?>");
			return
		}
	  /*if ( Main.requestCollction.resturent_address1  == ""){
		...
		if ( Main.requestCollction.resturent_postcode  == ""){
			alert("<?= $lang_resource['RESTURENT_POSTCODE_VV1'] ?>");
			return
		}
		if ( Main.requestCollction.resturent_collection_time  == ""){
			alert("<?= $lang_resource['RESTURENT_CNO_VV1'] ?>");
			return
		}				
		if($("#requestClooectioncheckremember").prop('checked') == false){
				alert("<?= $lang_resource['RESTURENT_CHK_VV1'] ?>");
			return
		}
		...
		geocoder.geocode( { 'address': address}, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK){
				...
				alert("Sorry, we were unable to geocode the resurant address");
			}
	        });
		}else{
			alert("Sorry, we were unable to geocode the customer address");
		}
	});
},
        ...
        { 
			            ...
			            $.post("panel/login/login.php", "type=social&data=" + JSON.stringify(d), function (e)
            {
                ...
                if (e != "")
                {
                    if (e == "suspended")
                    {
                        alert("<?= $lang_resource['Account_suspended_V2'] ?>");
                        return
                    }
...
LoginBox: function (){
	...
	$.post("panel/lib/front-bulk.php",'data=[{"operation":"FetchRequestCollectionSettings"}]', function (bb2){
		Main.Ready();
		bb5=JSON.parse(bb2);
			Main.RequestCollectionSettings = bb5.RequestCollectionSettings;


		$.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchRequestCollectionSettings"},{"operation":"GetSiteScheduleSettingsForSchedule"},{"operation":"GetRequestCollectionSettingsForSchedule"},{"operation":"FetchCheckoutInfo"},{"operation":"FetchTimeByZoneForSchedule"},{"operation":"GetSiteScheduleTextSettingsForSchedule"},{"operation":"FetchAllbusinessSettingDataCommon"},{"operation":"FetchAllFranchisesData"},{"operation":"FetchAllCountriesData"},{"operation":"FetchNeighbourhoodSettings"},{"operation":"FetchDecimalPoint"},{"operation":"FetchItemPointPermission"},{"operation":"FetchGlobalPointSettings"},{"operation":"getAutocomplete"} ,{"operation":"FetchMapLatlong"} ]', function (bb1){
		...
		if(available==true){
			var htms = '<div class="pop-dv" id="pop-dv-one">';
			...
			htms += '</table>'; 
			htms += ' </div>';
			htms += '<div class="pop-footer"><button type="button" class="rakesh" onclick="Popup.OnCancel()"><?= $lang_resource["SOUNDOK_V2"] ?></button>';
			htms += '</div>';       
			htms += '</div>';   
			htms += ' </div>';
			htms += '</div>';
			  
			Popup.Show(662, 662, htms, null, function (){
...
//site schedule End
    ...
    Main.Aid = a;
    Main.Loading();
    $.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchUserInfo"},{"operation":"FetchAllWidgetSettingsData"},{"operation":"FetchAllCuisineDataFront"},{"operation":"FetchAllReviews"},{"operation":"fetchenterprise"},{"operation":"FetchPanelInfo"},{"operation":"FetchSettingFront"},{"operation":"FetchZipMAxMin"},{"operation":"FetchSiteSetting"},{"operation":"FetchCountryCity"},{"operation":"FetchMostPopularsData"},{"operation":"FetchMostPopularsCategoryData"},{"operation":"FetchWhereAmIData"},{"operation":"FetchRecentActivityData"},{"operation":"FetchAllpanelFooterCustom"},{"operation":"FetchAllsettingsCustomFacebookNew"},{"operation":"FetchDefaultLangName"},{"operation":"FetchPageSettings"},{"operation":"FetchAllCountriesData"},{"operation":"FetchUserPointsData"},{"operation":"FetchLandingPageSettings"}]', function (b){
		...
		if(Main.WidgetSettings.default_settings.direct_business){
						//alert(JSON.stringify(Main.WidgetSettings.default_settings.default_business))
	...
	OpenWhereAmIDeliveryOfCityBox: function ()
    {
        ...
        $.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchAllCountriesData"}]', function (b)
        {
            ...
            if (b != "")
            {
				...
				if(viewDevice == "Desktop") {
				  Main.WhereAmIDeliveryOfCity(Main.WhereAmIData)
			} else if (viewDevice == "Mobile") {
...
PreorderMain: function (e)
    {
        ...
        var a= '<div class="popup_wrapper">';
		a += '<div class="pop_header">';
        a += '<div class="pop_heading"><h3><?=$lang_resource['CHOOSEDELTIMEPREORDER']?></h3></div>';
		 a += '<div class="pull_right" style="margin:8px 8px 0px 0px">';
		...
		var hhs = new Array();
			...
			var mmp = new Array();
			 mmp.push(JSON.parse('{"id":"","caption":"MM"}'));
			  ...
			  a += '<tr>';
         a += '<td align="left" colspan="2" style="padding: 10px 15px;"><span class="pop_label"><?=$lang_resource['PREORDERDATE']?>  </span></td>';
         a += '</tr>';
			 a += '<tr>';
         ...
         a += '</tr>';
		  a += '<tr>';
         a += '<td align="left" style="padding: 10px 15px; width:287px;">'+ Forms.CreateSelectPropertyPopup("recover14", "preorderhh", hhs,"", true,"Main.MinuteCheck(this)")+'</td>';
         a += '<td align="left" style="padding: 10px 15px;">'+ Forms.CreateSelectPropertyPopup("recover14", "preordermin", mmp,"", true)+'</td>';
         a += '</tr>';
		 a += '<tr>';
         a += '<td align="center" colspan="2" style="padding: 10px 15px;"><button type="button" class="pop_submit_btn" onclick="Main.preorderAction('+e.menuonlyeach[0].business+')"><?= $lang_resource['FRONT_MY_SUBMIT'] ?></button></td>';
         a += '</tr>';
...
PreOrderMenuCatalogFetch: function (d,pem) {
	...
	$(window).scrollTop(200);
               ...
               Main.Aid = b;
               Main.Loading();
	$.post("panel/lib/front-bulk.php", 'data=[{"operation":"pickupDeliverytime","businessid":' + d + '},{"operation":"timescdule","businessid":' + d + '},{"operation":"FetchBusinessOnlyMenu","businessid":' + d + "}]", function (a)
       {
		   Main.Ready();
		   	 a = JSON.parse(a);
			...
			a = JSON.stringify(a);
		     ...
		     if(viewDevice == "Mobile") {
				RestMenuList.PreOrderMenuCatalogFetchMob(a,d)
		 		   ...
		 		   Main.PreOrderMenuCatalog(a,d,pem)
			}
...
GeolocationID: function(){
		...
		if(Main.WhereAmIData.country) {
			...
			custom_link +=   "_delivery";
			 ...
			 Main.locatserch = JSON.parse(Main.WhereAmIData.location)
				console.log(JSON.stringify(Main.locatserch));
			custom_link +=   "_"+Main.locatserch.latitud;
			custom_link +=   "_"+Main.locatserch.longitud;
			if(Main.locatserch.zipcode){
				custom_link +=   "_"+Main.locatserch.zipcode;
			}else{
				 custom_link +=   "_-1";
			}
			
			custom_link +=   "_"+Main.locatserch.zoom;
			 ...
			 $.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchAllneighborhoodData","filters":[{"modifier":"neighborhood","name":"country","operator":"=","value":"' + c + '"},{"modifier":"neighborhood","name":"city","operator":"=","value":"' + b + '"}]}]', function (f)
        {
           ...
           /* if (a != Main.Aid)
			...
			var a1 = JSON.parse(f);
				...
				//alert(JSON.parse(f).neighborhood)
			  Main.neighborhood = JSON.parse(f).neighborhood;
			  ...
			  var t = "";
				...
				if((colony) && (colony==Main.neighborhood[d].id)) //edit
				{
					t = "selected";
				}
...
if(Main.settingfront.tab_delivery_neighborhood == 'f'){ 
    htms +='<div class="field" '+add_f+'>';
				  ...
				  htms +='<button type="button" class="geo_btn" style="background-color:#dcdcdc"  onclick="Main.GetUserLocation1()"><img src="images/homeimage/add-icon.png"></button>';
				  htms += '</div>'
...
if(Main.settingfront.tab_delivery_address == 'f'){ 
	...
	WhereAmIDelivery: function (c,options,comments,optionsid,total_price){
		...
		htms +='<table width="100%" border="0" cellspacing="0" cellpadding="0" class="pop_tbl" style=" margin:0px 0px 0px 0px">';
		if(counter != 0){
...
if(Main.User){
	...
	SaveWhereAmI: function (){
		...
		if(deliveryAccept == "1"){
			...
			//alert(Forms.Form.whereami.fields.cuisines.value)
		...
		if(Forms.Form.whereami.fields.location.value) {
			 Main.locatserch = JSON.parse(Forms.Form.whereami.fields.location.value)
	...
	if(deliveryAccept == 1){
		...
		$.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchzipcodeValidation","zipcode":"' + postcode + '"}]', function (tt){		   
			if (tt != ""){
				...
				if(Main.locatserch.zipcode){
					Main.WhereAmIData.zipcode = Main.locatserch.zipcode
				}
				Main.Ready();
				$("#top").html("");
				window.history.pushState( {"id":100} , "Business list", custom_link );
			...
			dd = dd.replace("-","")
					...
					$.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchBusinessPreOrderMenu","businessid":'+Shopping.ActiveBusiness+',"date":' + dd + ',"hour":' + Shopping.Cart.preordertimehh + ',"minute":' + Shopping.Cart.preordertimemm + ',"deliverytype":' + JSON.stringify(deldis) + "}]", function (a){
						...
						if(M!=-1){
							...
							Shopping.AddToCart('',Main.currentItem,op,cm,ops,totl,Main.Itemquantitysec)
						}else{
...
var geocoder = new google.maps.Geocoder();
		...
		CommonAccount: function ()
	{
		 ...
		 console.log("bus-own-setting"+Main.settingfront.business_owner_register)
    ...
    EditAccount: function (a)
    {
        ...
        if (a)
        {
            ...
            $.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchAllCountriesData"}]', function (c)
            {
				...
				/*$.post("panel/lib/front-bulk.php", 'data=[{"operation":"FetchAllcity"}]', function (c)
       			 {*/
    ...
    PreUserForm: function ()
    {
      ...
      //  Uploader.Init("user", "uform", null, true, Main.UserImageUploadFinished, Main.UserImageSelected, Main.UserImageStartUpload, "popup");
    ...
    TryToStartSaveUser: function (a){ 
		...
		/*else if(document.getElementById("colony").value == "")
        {
            alert("<?=$lang_resource['FRONT_ENTER_YOUR_COLONY'];?>")
		...
		else if(document.getElementById("api").value == "")
        {
        	swal("Error","<?= $lang_resource['FRONT_ENTER_YOUR_APT'] ?>","error");
				...
				{ 
					...
					if(viewDevice != "Mobile"){  
						if (d.trim() == "ok"){
							...
							alert("<?= $lang_resource['email_repeated_V2'] ?>")
	...
	ChooseTypeRegister: function ()
	{
		...
		{
			swal("Error","<?= $lang_resource['BUSINESS_OWNER_VALIDATION_MSG'] ?>","error");
			return false;
		}
    ...
    { 
		...
		//alert(document.getElementById("countryregister").value)
		...
		//alert(Forms.Form.user.fields.countryregister.value)
		 ...
		 Forms.CreateValue("user", "city", "");
		  ...
		  /* Forms.CreateValue("user", "colony", "");
		  Forms.UpdateValue("user", "colony", Forms.Form.user.fields.colony.value)
*/
		...
		//delete Forms.Form.user.fields.colony;
		...
		/*if(Forms.Form.user.fields.imgupload0)
		 {
			 
			delete Forms.Form.user.fields.imgupload0; 
		 }*/
      ...
      //  Forms.PrepareForSaving("user");
        ...
        Main.Loading(true);
		if(viewDevice == "Mobile"){
			  Main.loginid=Forms.Form.user.fields.email.value;
			  Main.passid=	Forms.Form.user.fields.pwd.value;
        ...
        $.post("panel/lib/front-main.php", "f=SaveUser&data=" + JSON.stringify(Forms.Form.user), function (Data)
        {
			...
			Main.userlevelk = JSON.stringify(Data.userlevel)

            Main.Ready();
			if(viewDevice == "Mobile"){
			 Main.positionregister=true;
  ...
  Main.code=document.getElementById("smscode").value;
    ...
    CheckOrder: function (c){
        ...
        if(a){
        	...
        	alert("<?= $lang_resource['FRONT_TRACK_ORDER'] ?>")
        }
    ...
    ShowOrderPop: function()
    {
		$(".popdiv").hide();
	if(Main.ordpopdt != "") {
	 Popup.Show(700, 700, Main.ordpopdt, null, function ()
        {
            ...
            else
            {
                ...
                { 
                    ...
                    if(sp.trim()=="ok")
                    {
                        ...
                        alert("<?= $lang_resource['PASSWORD_SENT1'] ?>")
						Popup.Close();
						if(viewDevice == "Mobile"){
						Main.InitInterface();
						}
                    ...
                    {
                        alert("<?= $lang_resource['EMAIL_INCORRECT'] ?>")
                    }
            ...
            else
            {
                ...
                { 
                    ...
                    if(sp.trim()=="ok")
                    {
                        ...
                        alert("<?= $lang_resource['PASSWORD_SENT'] ?>")
						Popup.Close();
						if(viewDevice == "Desktop"){
						Main.InitInterface();
						}
                    ...
                    {
                        alert("<?= $lang_resource['PASSWORD_INCORRECT'] ?>")
                    }
	...
	GetPropertyValueOnPropertyValueFoundForIdsindex: function (b, a, d)
	{
		for (var c in b)
		{
			var arr = b[c][a];
...
if(cuisinesstr.trim()==''){
	...
	SearchLocation2ndSave: function(){
		...
		if (Forms.Form.whereamiress.fields.rmin1.value == ""){
        ...
        Main.WhereAmIData.rmin = Forms.Form.whereamiress.fields.rmin1.value;
	...
	SubscribeEmail: function ()
		...
		if(postcode=="")
		{
			alert("<?= $lang_resource['ALERT_VALID_POSTAL_CODE_NON_EMPTY'] ?>")
			return false;
		}
							...
							if (tt != ""){
								...
								if(Main.zipcodeValidation1["value"] !=""){									
									...
									swal("Error","<?= $lang_resource['ALERT_VALID_POSTAL_CODE_UK'] ?>","error");									
										...
										if(term==false){
											alert("<?= $lang_resource['ALERT_TERM_NOT_CHECKED'] ?>")
											return false;
										}else{
	...
	var cc ='<iframe src="../../contact_us.php" width="100%" height="950" style="border: none"></iframe>';
