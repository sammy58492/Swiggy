<?php
	 ...
	 $printer_model = $psel_arr[0];
	  $deliveryhours = $order->buyer->deliveryhours;
	...
	$deliverydate = $order->buyer->deliverydate;
	$tdate=strtotime($deliverydate);
	if(is_numeric($tdate)){
	$deld = '';
	$deliverydate=date('d-m-Y',$tdate);
	}else{
	$deld = 'AS';
		if($deliverydate == "ASAP"){
		$deliverydate=$lang_resource['CHECKOUT_ASAP'];	
		}else{
	 ...
	 if($printer_model == 1)
		...
		$proNO = $val->name.Margeslash55($val->options);
		else
		$proNO = $val->name;
	  ...
	  $count_dish++;
		...
		if($deld!='AS')
		{
			...
			$dd = $deliveryhours.':'.$deliveryminute.' '.$deliverydate;
			$delivery_hrs = $dd.";";
		}
		else
		{
			$delivery_hrs = $deliverydate.";";
		}
		...
		$order->buyer->name = str_replace('%20',' ',$order->buyer->name);
		...
		else if($order->business[0]->paymethod->stripe==true){
			
				$string_val.="6;Credit Card;";
...
//File generate here		 
	...
	if($lasti == 0 ) {
	$html =' '.$each_record->optionheader.':';
	$html .=$each_record->optionchoice.',';
	}
