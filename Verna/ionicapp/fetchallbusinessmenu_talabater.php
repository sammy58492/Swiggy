...
pg_prepare($link,'sql432','SELECT * FROM w_menus WHERE business=$1 and enabled=true');
	...
	while($row = pg_fetch_array($result)) {
		...
		while($row1 = pg_fetch_array($result1))
			...
			$dish->id = $dishid;
			$dish->name =$row1['name'];
			$dish->name = dishidnamebylang($dishid, $lang);
			...
			$dish->description = dishiddescriptionbylang($dishid, $lang);
	...
	pg_prepare($link,'sqlcat'.$business,'SELECT * FROM w_categories WHERE business=$1 and enabled=true');
	$result12 = pg_execute($link,'sqlcat'.$business,array($business));
		while($row12 = pg_fetch_array($result12))
			{
				...
				$cat->id = $row12['id'];
				$cat->name = $row12['name'];
        $cat->name = businesscuisinenamebylang($row12['id'], $lang);
				array_push($categorynameid,$cat);
			}
		$catlog->dish =  $dishes;
		$catlog->name = businessmenunamebylang($row['id'], $lang);
